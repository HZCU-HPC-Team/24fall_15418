...
void clampedExpVector(float *values, int *exponents, float *output, int N)
{
	...
	const int VW = VECTOR_WIDTH;
	const __cmu418_vec_int ZERO = _cmu418_vset_int(0);
	__cmu418_mask fullMask = _cmu418_init_ones(VW);

	for (int i = 0; i < N - N % VW; i += VW)
	{
		__cmu418_vec_float x;
		_cmu418_vload_float(x, &values[i], fullMask);
		__cmu418_vec_float result = _cmu418_vset_float(1.0f);
		__cmu418_vec_int y;
		_cmu418_vload_int(y,&exponents[i],fullMask);
